...
- **Optional ModConfig support** — if [ModConfig](https://www.nexusmods.com/slaythespire2/mods/27) is installed, the following options appear in the `Mods` settings tab (changes apply immediately):
  - *Hide on ? (Unknown) nodes* — suppress the badge on Unknown nodes
  - *Badge position* — Right / Left / Above / Below
  - The mod runs fine without ModConfig (the toggle UI is simply skipped)
